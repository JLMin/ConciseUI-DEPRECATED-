...
function UpdateGreatWorks()
  ...
  local cui_tmpBuildings:table = {}; -- CUI
      ...
      for buildingInfo in GameInfo.Buildings() do
        ...
        if(pCityBldgs:HasBuilding(buildingIndex)) then
          ...
          if (numSlots ~= nil and numSlots > 0) then
            ...
            table.insert(cui_tmpBuildings, {Type=buildingType, Index=buildingIndex, CityBldgs=pCityBldgs, City=pCity, Info=buildingInfo});
          end
        end
      end
    end
  ...
  -- CUI: sort
  local IsSortByCity:boolean = CuiSettings:GetBoolean(CuiSettings.SORT_BY_CITY);
  if not IsSortByCity then
    table.sort(cui_tmpBuildings,
      function(a, b)
        if a.Index == b.Index then return a.City:GetID() < b.City:GetID();
        else return a.Index < b.Index; end
      end);
  end
  for _, item in ipairs(cui_tmpBuildings) do
    local instance:table = m_GreatWorkSlotsIM:GetInstance();
    local greatWorks:number = PopulateGreatWorkSlot(instance, item.City, item.CityBldgs, item.Info);
...
function OnMouseMove()
  if m_GreatWorkSelected ~= nil then
    ...
    mouseX = mouseX - (screenWidth - 1276) / 2; -- CUI
...
-- CUI =======================================================================
function CuiSetThemeMask(instance:table, srcBldgs:table, srcBuilding:number, srcSlot:number)
  local srcGreatWork:number = srcBldgs:GetGreatWorkInSlot(srcBuilding, srcSlot);
  local srcGreatWorkType:number = srcBldgs:GetGreatWorkTypeFromIndex(srcGreatWork);
  local srcGreatWorkObjectType:string = GameInfo.GreatWorks[srcGreatWorkType].GreatWorkObjectType;
  local greatWorkInfo:table = GameInfo.GreatWorks[srcGreatWorkType];
...
-- CUI =======================================================================
function CuiSetLockMask(instance:table, srcBldgs:table, srcBuilding:number, srcSlot:number)
  local srcGreatWork:number = srcBldgs:GetGreatWorkInSlot(srcBuilding, srcSlot);
  local srcGreatWorkType:number = srcBldgs:GetGreatWorkTypeFromIndex(srcGreatWork);
  local srcGreatWorkObjectType:string = GameInfo.GreatWorks[srcGreatWorkType].GreatWorkObjectType;
  ...
  -- lock with turns
  elseif (srcGreatWorkObjectType == "GREATWORKOBJECT_SCULPTURE"  or
          srcGreatWorkObjectType == "GREATWORKOBJECT_LANDSCAPE"  or
          srcGreatWorkObjectType == "GREATWORKOBJECT_PORTRAIT"   or
          srcGreatWorkObjectType == "GREATWORKOBJECT_RELIGIOUS") then
    local iTurnCreated:number = srcBldgs:GetTurnFromIndex(srcGreatWork);
...
function CuiOnSortButtonClick()
  local IsSortByCity:boolean = CuiSettings:ReverseAndGetBoolean(CuiSettings.SORT_BY_CITY);
