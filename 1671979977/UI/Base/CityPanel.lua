...
function OnInterfaceModeChanged( eOldMode:number, eNewMode:number )	
  ...
  if eOldMode == InterfaceModeTypes.CITY_MANAGEMENT then
    -- CUI: combine buttons.
    if Controls.ManageTileCheck:IsChecked() then Controls.ManageTileCheck:SetAndCall( false ); end
...
-- CUI =======================================================================
function CuiGetFoodToolTip(data:table)
  local foodToolTip:string =
    -- vanila tooltip
    data.FoodPerTurnToolTip .. "[NEWLINE][NEWLINE]" ..
    -- food consumption
    toPlusMinusString(-(data.FoodPerTurn - data.FoodSurplus)) .. " " ..
    Locale.Lookup("LOC_HUD_CITY_FOOD_CONSUMPTION") .. "[NEWLINE]------------------[NEWLINE]" ..
    -- happiness
  ...
  if data.Occupied then
    foodToolTip = foodToolTip .. "[NEWLINE]" ..
      "x" .. data.OccupationMultiplier ..
...
function CuiForceHideCityBanner()
  CuiSettings:SetBoolean(CuiSettings.SHOW_CITY_BANNER, false);
...
function Initialize()
  ...
  Controls.ManageTileCheck:RegisterCallback( Mouse.eMouseEnter, function() UI.PlaySound("Main_Menu_Mouse_Over"); end);
