...
-- CUI: icon instance
local m_SuzerainIM:table = InstanceManager:new("CuiSuzerainInstance", "Top", Controls.SuzerainStack)
...
function ViewList()
  ...
  local cui_Envoys:number = 0;
  local cui_Suzerain:number = 0;
  local cui_SuzerainList:table = {
  ...
  for _, item in CuiSortedTable(cui_SuzerainList, function(t, a, b) return t[a].idx < t[b].idx; end) do
    local cui_sInst:table = m_SuzerainIM:GetInstance();
  ...
  if m_mode == MODE.SendEnvoys then
    -- Controls.BonusArea:SetHide( false ); -- CUI: hide bonus area
  ...
  Controls.BonusArea:SetHide( true );
  if m_mode == MODE.SendEnvoys then
    local confirmAreaY:number = Controls.ConfirmFrame:GetSizeY();
    Controls.ConfirmFrame:SetHide( false );
    Controls.TotalFrame:SetOffsetY(70);
    Controls.CityStateScroll:SetSizeY( m_height - 268 - confirmAreaY ); -- 208 + 60
  else
    Controls.ConfirmFrame:SetHide( true );
