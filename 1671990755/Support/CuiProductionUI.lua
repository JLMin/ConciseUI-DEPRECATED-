...
function PopulateGroup(groupIM, itemIM, data, group, mode)
  ...
  if not isNil(groupInstance) then
    ...
    if     group == GROUP.DISTRICT then OnSizeChangeFunc = OnBuildingListSizeChanged;
    elseif group == GROUP.WONDER   then OnSizeChangeFunc = OnWonderListSizeChanged;
    elseif group == GROUP.UNIT     then OnSizeChangeFunc = OnUnitListSizeChanged; end
...
function PopulateProduceItem(parent, itemIM, item, mode)
  ...
  if skipItem then canPurchase = CanPurchaseItem(item);
  ...
  if item.IsDistrict then
    local completed = not item.Enable and item.HasBeenBuilt and item.OnePerCity and not item.IsPillaged;
    instance.CompletedArea:SetHide(not completed);

    local turnsStr = "";
    local checkMark = completed and not item.Contaminated and item.Progress == 0;
    if     checkMark         then turnsStr = "[ICON_Checkmark]";
    elseif item.Contaminated then turnsStr = TurnString(item.ContaminatedTurns);
                             else turnsStr = TurnString(item.Turns);
    end
...
function PopulatePurchaseItem(parent, itemIM, item, mode)
  ...
  if mode == MODE.GOLD or mode == MODE.FAITH then
    ...
    if     item.GoldUnlock  then isG = true;
    elseif item.FaithUnlock then isG = false;
                            else return; end
  end
  ...
  local enable = false;
  if isG then enable = item.GoldEnable; else enable = item.FaithEnable; end
  ...
  local cost = -1;
  if isG then cost = item.GoldCost; else cost = item.FaithCost; end
...
function RegisterProduceButtons(instance, item)
  instance.Button:RegisterCallback(Mouse.eRClick,
    function() LuaEvents.OpenCivilopedia(item.Type); end
  );
  --
  if item.IsDistrict then
    instance.Button:RegisterCallback(Mouse.eLClick,
      function() OnBuildDistrict(m_city, item); end
    );

    instance.GoldButton:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseDistrict(m_city, item, Y_GOLD); end
    );

    instance.FaithButton:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseDistrict(m_city, item, Y_FAITH); end
    );
  end
  --
  if item.IsBuilding or item.IsWonder then
    instance.Button:RegisterCallback(Mouse.eLClick,
      function() OnBuildBuilding(m_city, item); end
    );

    instance.GoldButton:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseBuilding(m_city, item, Y_GOLD); end
    );

    instance.FaithButton:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseBuilding(m_city, item, Y_FAITH); end
    );
  end
  --
  if item.IsUnit then
    local formation;
    if     item.IsStandard then formation = UNIT_STANDARD;
    elseif item.IsCorps    then formation = UNIT_CORPS;
    elseif item.IsArmy     then formation = UNIT_ARMY;
    end

    instance.Button:RegisterCallback(Mouse.eLClick,
      function() OnBuildUnit(m_city, item, formation); end
    );

    instance.GoldButton:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseUnit(m_city, item, formation, Y_GOLD); end
    );

    instance.FaithButton:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseUnit(m_city, item, formation, Y_FAITH);end
    );
  end
  --
  if item.IsProject then
...
function RegisterPurchaseButtons(instance, item, yield)
  ...
  if item.IsDistrict then
    instance.Button:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseDistrict(m_city, item, yield); end
    );
  end
  --
  if item.IsBuilding or item.IsWonder then
    instance.Button:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseBuilding(m_city, item, yield); end
    );
  end
  --
  if item.IsUnit then
    ...
    instance.Button:RegisterCallback(Mouse.eLClick,
      function() OnPurchaseUnit(m_city, item, formation, yield); end
    );
  end
  --
  if item.IsProject then
...
function OnPurchaseDistrict(city, districtEntry, yield)
  ...
  if bNeedsPlacement then
    local tParameters = {};
    tParameters[CityOperationTypes.PARAM_DISTRICT_TYPE] = districtEntry.Hash;
    tParameters[CityCommandTypes.PARAM_YIELD_TYPE] = GameInfo.Yields[yield].Index;
    UI.SetInterfaceMode(InterfaceModeTypes.DISTRICT_PLACEMENT, tParameters);
  else
    local tParameters = {};
    tParameters[CityOperationTypes.PARAM_DISTRICT_TYPE] = districtEntry.Hash;
    ...
    if yield == Y_GOLD then UI.PlaySound("Purchase_With_Gold");
                       else UI.PlaySound("Purchase_With_Faith");
    end
  end
...
function ItemDisabledInQueue(item, queue)
  ...
  if item.IsProject  then return IsItemInQueue(item, queue) and item.IsRepeatable; end
