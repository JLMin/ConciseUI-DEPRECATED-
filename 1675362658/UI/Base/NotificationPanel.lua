...
local SCROLLBAR_OFFSET						:number = 0; -- CUI
...
local CuiIconColors = {
  ...
  -- vanilla
  BARBARIANS_SIGHTED       = "Military",
  DIPLOMACY_SESSION        = "Marine",
  PLAYER_DEFEATED          = "Red",
  WONDER_COMPLETED         = "Gold",
  CITY_RANGE_ATTACK        = "Civ6Red",
  UNIT_PROMOTION_AVAILABLE = "Civ6Green",
...
-- CUI =======================================================================
function CuiGetColorByName(name:string)
  name = string.gsub(name, "ICON_", "");
  name = string.gsub(name, "NOTIFICATION_", "");
  if     string.sub(name, 1,4) == "SPY_"     then return CuiIconColors["SPY"];
  elseif string.sub(name, 1,8) == "VOLCANO_" then return CuiIconColors["VOLCANO"];
  elseif string.sub(name, 1,6) == "STORM_"   then return CuiIconColors["STORM"];
  elseif string.sub(name, 1,6) == "RIVER_"   then return CuiIconColors["RIVER"];
  elseif string.sub(name, 1,8) == "DROUGHT_" then return CuiIconColors["DROUGHT"];
  elseif string.sub(name, 1,8) == "NUCLEAR_" then return CuiIconColors["NUCLEAR"];
  elseif string.sub(name, 1,6) == "DIPLO_"   then return CuiIconColors["DIPLO"];
  elseif CuiIconColors[name]                 then return CuiIconColors[name];
  else                                            return "White";
  end
end
...
function GetDefaultHandler()
	return g_notificationHandlers[NotificationTypes.DEFAULT];
end
...
function OnDefaultAddNotification( pNotification:table )
	...
	if ( table.count(notificationEntry.m_IDs)==1 
		...
		if notificationEntry.m_Instance then
          ...
          -- CUI, custom color
          local color = CuiGetColorByName(notificationEntry.m_IconName);
          notificationEntry.m_Instance.Icon:SetColorByName(color);
				end
			else
				if(notificationEntry.m_TypeName ~= nil) then
					...
					if (textureOffsetX ~= nil) then
						notificationEntry.m_Instance.Icon:SetTexture( textureOffsetX, textureOffsetY, textureSheet );
            -- CUI, custom color
            local color = CuiGetColorByName(notificationEntry.m_TypeName);
            notificationEntry.m_Instance.Icon:SetColorByName(color);
					end
				end
			end
