...
function RefreshTopPanel()
  ...
  if destinationCity ~= nil and originCity ~= nil then
    ...
    local oBackColor:number, oFrontColor:number  = UI.GetPlayerColors( originCity:GetOwner() );
    ...
    if (destinationMajorityReligion > 0) then
      ...
      if (pressureValue ~= 0) then
        ...
        originTooltipText = originTooltipText .. sourceText;
        AddReligiousPressureResourceEntry(GameInfo.Religions[destinationMajorityReligion], pressureValue, true, sourceText, Controls.OriginReligiousPressure);
        originReceivedResources = true;
      end
    end
    ...
    Controls.OriginResources:RegisterSizeChanged( function() ResizeResourceBackgroundColumns(Controls.OriginResources, Controls.OriginResourcesLeftColumn, Controls.OriginResourcesMidColumn, Controls.OriginResourcesRightColumn); end );
    ...
    if (originMajorityReligion > 0) then
      ...
      if (pressureValue ~= 0) then
        ...
        destinationTooltipText = destinationTooltipText .. sourceText;
        AddReligiousPressureResourceEntry(GameInfo.Religions[originMajorityReligion], pressureValue, false, sourceText, Controls.DestinationReligiousPressure);
        destinationReceivedResources = true;
      end
    end
    ...
    Controls.DestinationResources:RegisterSizeChanged( function() ResizeResourceBackgroundColumns(Controls.DestinationResources, Controls.DestinationResourcesLeftColumn, Controls.DestinationResourcesMidColumn, Controls.DestinationResourcesRightColumn); end );
...
function AddCityToDestinationStack(city:table)
  ...
  -- Update distance to city
  -- CUI: get actual turns
  local tradeRouteInfo:table = CuiGetTradeRouteInfo(originCity, city);
  ...
  if (cityMajorityReligion > 0) then
    ...
    if (pressureValue ~= 0) then
      ...
      tooltipText = tooltipText .. sourceText;
      AddReligiousPressureResourceEntry(GameInfo.Religions[cityMajorityReligion], pressureValue, true, sourceText, cityEntry.ReligiousPressure);
    end
  end
  ...
  cityEntry.ResourceInfoGrid:RegisterSizeChanged( function() ResizeResourceBackgroundColumns(cityEntry.ResourceInfoGrid, cityEntry.ResourceInfoLeftColumn, cityEntry.ResourceInfoMidColumn, cityEntry.ResourceInfoRightColumn); end );
end
...
-- ===========================================================================
function ResizeResourceBackgroundColumns(parent:table, leftColumn:table, midColumn:table, rightColumn:table)
...
function CuiSortDestinations()  
  if cui_sortBy ~= -1 then CuiSortByYield(cui_sortBy); end
end

-- CUI ========================================================================
function CuiSortByYield(yieldID:number)
  local yieldValues:table = {};
  for _, cityID in ipairs(m_filteredDestinations) do
    local value:number = CuiGetYieldValue(cityID, yieldID);
    local item:table = {};
    item.id  = cityID;
    item.val = value;
  ...
  m_filteredDestinations = {};
  for _, item in CuiSortedTable(yieldValues, function(t, a, b) return t[a].val > t[b].val; end) do
    table.insert(m_filteredDestinations, item.id);
  end
end

-- CUI ========================================================================
function CuiGetYieldValue(cityID:number, yieldID:number)
  local entry:table  = m_unfilteredDestinations[cityID];
  local city:table   = nil;
  ...
  if value > 0 then return value; else return 0; end
end

-- CUI ========================================================================
function CuiOnSortButtonClick(yieldID:number)
