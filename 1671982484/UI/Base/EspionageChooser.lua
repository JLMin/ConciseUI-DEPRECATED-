...
local MAX_DISTRICTS_TO_SHOW:number = 8; -- CUI: before xp2
...
local cui_filterList:table = {};
local cui_playerList:table = {};
local cui_filterSelected:number = 1;
local cui_filterSelectedName:string = "LOC_ROUTECHOOSER_FILTER_ALL";
local cui_filteredCivs:table = {};

local CUI_AVAILABLE_DISTRICTS:table = {};
...
function AddDistrictIcons( kParentControl:table, pCity:table )
  ...
  for _, item in pairs(CUI_AVAILABLE_DISTRICTS) do
    local kDistrictInst:table = {};
    local pDistrict:table = CuiGetMatchedDistrict(item.dType, pCityDistricts);
    if pDistrict then
...
function AddDistrictIcon(kInstanceIM:table, pCity:table, pDistrict:table)
  ...
  kInstance.DistrictIcon:SetAlpha(1.0);
...
-- CUI =======================================================================
function CuiCivCheck(player:table)
  local localPlayer = Players[Game.GetLocalPlayer()];
...
-- CUI =======================================================================
function CuiOnFilterSelected(index:number, filterIndex:number)
...
-- CUI =======================================================================
function CuiFilterByCiv(civTypeID:number)
  -- Clear Filter
  cui_filteredCivs = {};
...
function CuiRefreshFilters()
  ...
  for i, player in ipairs(players) do
    local pPlayerInfluence:table = Players[player:GetID()]:GetInfluence();
...
-- CUI =======================================================================
function CuiGetFilterIndex(filterName:string)
  for index, filter in ipairs(cui_filterList) do
...
-- CUI =======================================================================
function CuiAddFilter(filterName:string, filterFunction)
...
-- CUI =======================================================================
function CuiBuildFilterEntry(filterIndex:number)
...
-- CUI =======================================================================
function CuiGetMatchedDistrict(dType:string, districts:table)  
...
-- CUI =======================================================================
function CuiAddEmptyDistrictIcon(kInstanceIM:table)
  local kInstance:table = kInstanceIM:GetInstance();
...
function CuiHasDistrict(city)
  if cui_sortBy == -1 then return true;
  else
    local dType:string = CUI_AVAILABLE_DISTRICTS[cui_sortBy].dType;
...
-- CUI =======================================================================
function CuiOnSortButtonClick(index:number)
