...
function RealizeMeterAndBoosts( kControl:table, kData:table )
  ...
  -- CUI: boost warining
  if kControl.MainPanel ~= nil then
    local couldRemind:boolean = kData.Boostable and (not kData.BoostTriggered) and ((kData.Progress + kData.BoostAmount) >= 1);
    if kData.TechType ~= nil then
      local techRemind:boolean = CuiSettings:GetBoolean(CuiSettings.TECK_REMINDER);
      if techRemind and couldRemind then
        kControl.MainPanel:SetTexture("CivicPanel_Frame");
        kControl.MainPanel:SetColorByName("StatGoodCS");
      else
        kControl.MainPanel:SetTexture("ResearchPanel_Frame");
        kControl.MainPanel:SetColor(UI.GetColorValueFromHexLiteral(0xFFFFFFFF));
      end
    elseif kData.CivicType ~= nil then
      local civicsRemind:boolean = CuiSettings:GetBoolean(CuiSettings.CIVICS_REMINDER);
      if civicsRemind and couldRemind then
        kControl.MainPanel:SetTexture("CivicPanel_Frame");
        kControl.MainPanel:SetColorByName("StatGoodCS");
      else
        kControl.MainPanel:SetTexture("CivicPanel_Frame");
