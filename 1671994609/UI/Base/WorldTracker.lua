...
function RealizeEmptyMessage()
  if(m_hideChat and m_hideCivics and m_hideResearch and m_hideGossipLog and m_hideCombatLog) then
...
function UpdateResearchPanel( isHideResearch:boolean )
  ...
  if kResearchData == nil then
    ...
    m_researchInstance.IconButton:SetToolTipString(Locale.Lookup(kResearchData.ToolTip)); -- CUI: add tooltip for research
  end
end
...
function UpdateCivicsPanel(hideCivics:boolean)
  ...
  if kCivicData == nil then
    ...
    m_civicsInstance.IconButton:SetToolTipString(Locale.Lookup(kCivicData.ToolTip)); -- CUI: add tooltip for civics
  end
end
